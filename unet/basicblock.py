...
def patch_embedding(input, upscale_factor):
    """
    A function to rearrange elements in a tensor of shape `(*, r^2C, H, W)` to
    a tensor of shape `(C, rH, rW)`. This can be seen as a reverse operation to
    PixelUnShuffle if interpreted in a certain way, but it's more akin to PixelShuffle.
    
    Parameters:
    - input: Input tensor.
    - upscale_factor: Factor to increase spatial dimensions.
    """
    batch_size, channels, in_height, in_width = input.size()
    out_channels = channels // (upscale_factor ** 2)

    # Reshape to group spatial information from channels
    input_view = input.view(batch_size, out_channels, upscale_factor, upscale_factor, in_height, in_width)
...
class PatchUnEmbedding(nn.Module):
    """
    A module for rearranging elements in a tensor of shape `(*, r^2C, H, W)` to
    a tensor of shape `(C, rH, rW)`.
    """
    def __init__(self, upscale_factor):
    ...
    def forward(self, input):
        return patch_embedding(input, self.upscale_factor)
